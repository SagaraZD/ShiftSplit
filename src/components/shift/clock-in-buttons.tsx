...
export function ClockInButtons({ onClockIn, submittingLocationId }: Props) {
  return (
        ...
        {OFFICE_GEOFENCES.map((office) => {
          ...
          return (
            <Pressable
              ...
              style={{ backgroundColor: office.color, opacity: submittingLocationId && !submitting ? 0.5 : 1 }}>
              {submitting ? <ActivityIndicator color="#fff" /> : <LogIn size={18} color="#fff" />}
